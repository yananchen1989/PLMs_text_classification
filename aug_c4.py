...
####################### generation setting ######################
    ...
    if args.dsn in ['ag','uci']:
        ...
        t5_ss = AutoModelWithLMHead.from_pretrained("{}/finetunes/t5_st_ss/epoch_2".format(PATH_SCRATCH))
        
        bart_pp = AutoModelWithLMHead.from_pretrained("{}/finetunes/bart_st_pp/epoch_5".format(PATH_SCRATCH))
        bart_ss= AutoModelWithLMHead.from_pretrained("{}/finetunes/bart_st_ss/epoch_5".format(PATH_SCRATCH))        

        gen_nlp['t5_st_ss']  = pipeline("text2text-generation", model=t5_ss, tokenizer=tokenizer_t5, device=0)
        gen_nlp['t5_st_pp']  = pipeline("text2text-generation", model=t5_pp, tokenizer=tokenizer_t5, device=0)
        gen_nlp['bart_st_ss']  = pipeline("text2text-generation", model=bart_ss, tokenizer=tokenizer_bart, device=0)
        gen_nlp['bart_st_pp']  = pipeline("text2text-generation", model=bart_pp, tokenizer=tokenizer_bart, device=0)
