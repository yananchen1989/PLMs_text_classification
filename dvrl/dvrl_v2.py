...
class Dvrl(object):
  ...
  def __init__(self, x_train, y_train, x_valid, y_valid,
               problem, pred_model, parameters, checkpoint_file_name, flags, seed):
    """Initializes DVRL."""
  ...
  def data_value_evaluator(self):
    ...
    with tf.compat.v1.variable_scope('data_value_estimator', reuse=tf.compat.v1.AUTO_REUSE):
      ...
      ################### alternative
      inter_layer = tf.compat.v1.layers.Dense(self.hidden_dim, activation="relu")(inputs)

      for _ in range(int(self.layer_number - 3)):
        inter_layer =  tf.compat.v1.layers.Dense(self.hidden_dim, activation="relu")(inter_layer)

      inter_layer =  tf.compat.v1.layers.Dense(self.comb_dim, activation="relu")(inter_layer)

      # Combines with y_hat
      comb_layer = tf.concat((inter_layer, self.y_hat_input), axis=1)
      comb_layer = tf.compat.v1.layers.Dense(self.comb_dim, activation="relu")(comb_layer)
      dve = tf.compat.v1.layers.Dense(1, activation="sigmoid")(comb_layer)
  ...
  def train_dvrl(self, perf_metric):
    ...
    prob = tf.reduce_sum(input_tensor=self.s_input * tf.math.log(est_data_value + self.epsilon) +\
                         (1-self.s_input) * \
                         tf.math.log(1 - est_data_value + self.epsilon))
