...
class Dvrl(object):
  ...
  def __init__(self, x_train, y_train, x_valid, y_valid,
    ...
    self.seed = random.sample(list(range(10000)), 1)[0]
  ...
  def data_value_evaluator(self):
    ...
    with tf.variable_scope('data_value_estimator', reuse=tf.AUTO_REUSE):
      ...
      ################### alternative
      inter_layer = tf.layers.Dense(self.hidden_dim, activation="relu")(inputs)

      for _ in range(int(self.layer_number - 3)):
        inter_layer =  tf.layers.Dense(self.hidden_dim, activation="relu")(inter_layer)

      inter_layer =  tf.layers.Dense(self.comb_dim, activation="relu")(inter_layer)

      # Combines with y_hat
      comb_layer = tf.concat((inter_layer, self.y_hat_input), axis=1)
      comb_layer = tf.layers.Dense(self.comb_dim, activation="relu")(comb_layer)
      dve = tf.layers.Dense(1, activation="sigmoid")(comb_layer)
  ...
  def train_dvrl(self, perf_metric):
    ...
    dve_loss = (-self.reward_input * prob) + \
                1e3 * (tf.maximum(tf.reduce_mean(est_data_value)  - self.threshold, 0) + \
                       tf.maximum((1-self.threshold) - tf.reduce_mean(est_data_value), 0))
