...
data_loading.preprocess_data(normalization, 'train.csv', 'valid.csv', 'test.csv')
...
ds.df_train, ds.df_valid = train_test_split(ds.df_train, test_size=0.3)
enc = encoder('dan','cpu')
...
pred_model.add(keras.layers.Dense(parameters['hidden_dim'],
                                activation='relu'))
pred_model.add(keras.layers.Dense(ds.df_test.label.unique().shape[0], activation='softmax'))
