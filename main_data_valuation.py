...
if args.dsn not in ['blog', 'adult']:
    ...
    data_loading.preprocess_data(args.normalization, \
        'train_{}.csv'.format(args.dsn), 'valid_{}.csv'.format(args.dsn), 'test_{}.csv'.format(args.dsn))
...
print('summary==>', args.dsn, ' '.join(['{}:{}'.format(k, v) for k, v in vars(args).items()]), \
   'auc:', metrics.auc(fpr, tpr), 'mean_dve_out:', sum(dve_out) / len(dve_out) )
